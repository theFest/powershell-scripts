Function FirewallManagerHNC {
    ...
    Param (
        ...
        [ValidateSet("AddRule", "RemoveRule", "ModifyRule", "EnableRule", "DisableRule", "BlockIP", "UnBlockIP", "BlockIP", "UnBlockIP")]
        [string]$Operate,

        [Parameter(Mandatory = $false, HelpMessage = "Enter a name for the Windows firewall rule")]
        ...
        [Parameter(Mandatory = $false, HelpMessage = "Via Windows Firewall API you can disable, enable or reset Windows Firewall")]
        [ValidateSet("EnableFW", "DisableFW", "ResetFW")]
        [string]$Manage,
        ...
        [ValidateSet("Any", "OutHost", "ExportFile")]
        [string]$OutputType,
    ...
    BEGIN {
        ...
        if ($OutputType) {
            $Outputs = {
                try {
                    $RulesList = $FirewallPolicy.Rules ; $RuleCount = $RulesList.Count
                    ...
                    switch ($Manage) {
                        ...
                        "OutFile" {
                            $RulesList | Export-Csv -Path $ExportPath -NoTypeInformation -Verbose
        ...
        if ($CheckForDuplicates) {
            ...
            $RemoveDuplicates = {
                ...
                Write-Warning -Message "Searching for duplicate Firewall rules, if found, we're removing them..."
        ...
        switch ($Manage) {
            "EnableFW" {
                $FirewallPolicy.EnableRuleGroup("Windows Firewall")
                Write-Host "Windows Firewall has been enabled" -ForegroundColor DarkGreen
            }
            "DisableFW" {
                $FirewallPolicy.DisableRuleGroup("Windows Firewall")
    ...
    PROCESS {
        ...
        if ($Action) {
            $FirewallProps.Action = @{Allow = 1; Block = 0 }[$Action]
        }
        if ($Direction) {
        ...
        switch ($Operate) {
            ...
            "BlockIP" {
                ...
                $Rule = $FirewallPolicy.Rules | Where-Object { $_.Name -eq "Block $IP" }
                if ($Rule) {
                    Write-Warning -Message "An IP blocking rule for $IP already exists"
                }
                else {
                    $NewRule = $FirewallPolicy.Rules.Create()
                    $NewRule.Name = "Block $IP"
                    $NewRule.Protocol = "Any"
                    $NewRule.Direction = "Inbound"
                    $NewRule.RemoteAddresses = $IP
                    $NewRule.Enabled = $true
                    $FirewallPolicy.Rules.Add($NewRule)
                    Write-Host "IP address $IP has been blocked by the Windows Firewall" -ForegroundColor DarkGreen
                }
            }
            "UnblockIP" {
                $IP = $IPAddress.IPAddressToString
                $Rule = $FirewallPolicy.Rules | Where-Object { $_.Name -eq "Block $IP" }
                if ($Rule) {
                    $FirewallPolicy.Rules.Remove($Rule)
                    Write-Host "IP address $IP has been unblocked by the Windows Firewall" -ForegroundColor DarkGreen
                }
                else {
                    Write-Warning -Message "No IP blocking rule exists for $IP"
                }
            }
            default {
                Write-Warning -Message "Invalid action, use: AddRule, RemoveRule, ModifyRule, EnableRule, DisableRule!"
        ...
        if ($CSVFilePath) {
            Import-Csv -Path $CSVFilePath | ForEach-Object {
                ...
                FirewallRulesManagerFWPolicy @params
            }
        }
    }
    END {
        Write-Output -InputObject $FirewallRule
        if ($OutputType) {
            Write-Host "Output information after tampering: " `
                -ForegroundColor Cyan ; pause ; Invoke-Command -ScriptBlock $Outputs
        }
        if ($CheckForDuplicates) {
