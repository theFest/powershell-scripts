Function RemoteImportRegKey {
    <#
    .SYNOPSIS
    Imports a registry key from a local or remote machine.
    
    .DESCRIPTION
    This function allows you to import a registry key from either a local or remote machine. It establishes a remote PowerShell session if the ComputerName parameter is provided, copies the registry file to the remote machine, imports the registry key using the 'reg.exe' command, and removes the copied file afterwards. If no ComputerName is specified, the function imports the registry key locally.
